#!/usr/bin/env python3
"""
Test 4: Graph captured at MAX, replayed alternating bs=1 / bs=MAX.
Worst case for buffer reuse — rapid switching between tiny and full batches.
"""
import sys, os
...
def main():
    ...
    with h.vllm_ctx:
        h.log(f"\n=== Alternating bs=1 / bs={capture_bs} ===")
        ...
        for i in range(NUM_REPLAYS):
            effective_bs = 1 if i % 2 == 0 else capture_bs
            ...
            if real_out.isnan().any().item() or real_out.isinf().any().item():
                nan_count += 1
                if nan_count <= 3:
                    h.log_err(f"REPLAY {i}: bs={effective_bs} "
                              f"NaN={real_out.isnan().sum().item()}/{real_out.numel()}")
            elif i % 50 == 0:
                h.log(f"  Replay {i}/{NUM_REPLAYS}: bs={effective_bs} OK")

        h.log_all(f"Alternating: {nan_count}/{NUM_REPLAYS} NaN")
        if nan_count > 0:
            h.teardown()
            sys.exit(1)
